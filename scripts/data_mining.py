...
ent_logger = hrai_logger(
# Constants
GOOGLE_API_KEY = os.getenv('GOOGLE_SEARCH_API_KEY')
GOOGLE_CSE_ID = os.getenv('GOOGLE_CSE_ID')
INPUT_FILE = 'scripts/input_data.csv'
OUTPUT_FILE = 'scripts/evaluated_data.csv'
MAX_WORKERS = 5
MAX_RETRIES = 3
RETRY_DELAY = 5
...
# Define the new system prompt for URL selection
URL_SELECTION_SYSTEM_PROMPT = """
You are an AI assistant that helps in selecting the most relevant URL for a company based on search results. 
You will be given a company name, a list of search results, and a query. 
Your task is to analyze the search results and select the most relevant URL for the company.
...
def evaluate_company(company_data, webpage_content, url):
    ...
    """
    return ask_gpt_with_retries(prompt)
...
def ask_gpt(message, system_prompt=None, api='openai'):
...
def ask_gpt_with_retries(prompt, system_prompt=SYSTEM_PROMPT, max_retries=MAX_RETRIES, base_delay=RETRY_DELAY):
    """Ask gpt a question and return the parsed JSON response, with retries."""
    for attempt in range(max_retries):
        try:
            response = ask_gpt(prompt, system_prompt)
            logger.debug(f"gpt API response: {response}")
            parsed_response = extract_json_from_text(response)
            if parsed_response:
                return parsed_response
            else:
                raise ValueError("No valid JSON found in gpt's response")
        except Exception as e:
            ...
            if attempt < max_retries - 1:
                ...
                raise Exception(f"Max retries reached for gpt API call: {str(e)}")

    raise Exception("Max retries reached for gpt API call")
...
def select_company_url(company_name, search_results, query):
    """Select the most relevant URL for the company using AI."""
    ...
    Provide your analysis in the specified JSON format.
    """
    return ask_gpt_with_retries(prompt, system_prompt=URL_SELECTION_SYSTEM_PROMPT)
